...
class PatrolBackgroundGpsHub {
  ...
  var _foregroundScanRelayEnabled = false;

  var _foregroundScanWantsBarometer = false;
  ...
  bool get _wantsBarometer =>
      _scanWantsBarometer || _foregroundScanWantsBarometer;

  /// Main-isolate foreground auto-scan — relays GPS without a second Geolocator stream.

  Future<void> setForegroundScanRelay({
    required bool enabled,

    bool wantsBarometer = false,
  }) async {
    _foregroundScanRelayEnabled = enabled;

    _foregroundScanWantsBarometer = enabled && wantsBarometer;
  ...
  Future<void> ensureRunning({bool? scanWantsBarometer}) async {
    ...
    final nextOptions = await _streamOptionsFromConfig(
      enableBarometer: _wantsBarometer,
    );

    if (_listening && _streamOptions != null) {
      final baroChanged =
          _streamOptions!.enableBarometer != nextOptions.enableBarometer;

      final streamParamsChanged = _streamParamsChanged(
        _streamOptions!,
        nextOptions,
      );
  ...
  Future<void> stop() async {
    ...
    _foregroundScanRelayEnabled = false;

    _foregroundScanWantsBarometer = false;
  ...
  void _dispatchEvent(SuperGpsEvent event) {
    ...
    if (_foregroundScanRelayEnabled) {
      PatrolFgsIsolateBridge.notifyScanGpsSampleFromFgs(event);
    }
